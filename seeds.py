...
# anytime we want to use a CRUD operation with SQLAlchemy, we need to establish a temporary session connection with the Session class.
db = Session()

#  insert Users - this doesn't get written to the database, it merely preps the SQL queries. 
db.add_all([
...
# to run INSERT statements, we need to call db.commit
db.commit()

# insert posts - must run after the insertion of users since post requires user_id
db.add_all([
